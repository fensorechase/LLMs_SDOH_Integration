...
def parse_config():
    ...
    parser.add_argument(
        "--task", type=str, default="domain_CTA", help="either topic_CTA or domain_CTA."
    )  ###
    parser.add_argument(
        "--feat_set",
        type=str,
        default="a",
        help="1 options: a, b, or c, 2 options: ab, ac, or bc, 3 options: abc",
    )  ###
    parser.add_argument(
        "--json_format",
        type=bool,
        default=False,
        help="False for freetext prompt. True for JSON prompt.",
    )  ###
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="either 0 for 0 shot, 1 for 1 shot, 3 for 3 shot.",
    )  ###
    ...
    parser.add_argument(
        "--max_gen_len", type=int, default=128, help=""
    )  # 64 (cut some responses off), 512, 4096

    parser.add_argument(
        "--input_data_file", type=str, default="INPUT_NaNDA.csv", help=""
    )  # INPUT_subset_NaNDA_domain_column_classification.csv
    parser.add_argument(
        "--output_data_file", type=str, required=False, default="", help=""
...
PROMPT_DICT = {
    ...
    "freetext_oneshot_prompt_input_a": '{oneshot_prefix}\n[INST] Variable name: "{variable_name}".\nDomain:[/INST]',
    "freetext_oneshot_prompt_input_b": '{oneshot_prefix}\n[INST] Variable label: "{variable_label}".\nDomain:[/INST]',
    "freetext_oneshot_prompt_input_c": '{oneshot_prefix}\n[INST] Data source: "{data_source}".\nDomain:[/INST]',
    "freetext_oneshot_prompt_input_ab": '{oneshot_prefix}\n[INST] Variable name: "{variable_name}".\nVariable description: "{variable_label}".\nDomain:[/INST].',
    "freetext_oneshot_prompt_input_ac": '{oneshot_prefix}\n[INST] Variable name: "{variable_name}".\nData source: "{data_source}".\nDomain:[/INST]',
    "freetext_oneshot_prompt_input_bc": '{oneshot_prefix}\n[INST] Variable description: "{variable_label}".\nData source: "{data_source}".\nDomain:[/INST]',
    "freetext_oneshot_prompt_input_abc": '{oneshot_prefix}\n[INST] Variable name: "{variable_name}".\nVariable description: "{variable_label}".\nData source: "{data_source}".\nDomain:[/INST]',
}
...
def generate_prompt(variable_name, variable_label, data_source):
    ...
    if args.num_shots == 0:
        prompt_input = (
            str(format_prefix) + "zeroshot_prompt_input_" + str(args.feat_set)
        )
        return system_prompt + PROMPT_DICT[prompt_input].format(
        ...
        oneshot_prefix = globals()[
            oneshot_prefix
        ]  # Grabs the value of the varaible with *name* x+"_ONE_SHOTS_FREETEXT"

        return system_prompt + PROMPT_DICT[prompt_input].format(
            oneshot_prefix=oneshot_prefix,
            variable_name=variable_name,
            variable_label=variable_label,
            data_source=data_source,
        )
...
def build_generator(
    ...
    )  # Previously: skip_special_tokens=False, clean_up_tokenization_spaces=False
